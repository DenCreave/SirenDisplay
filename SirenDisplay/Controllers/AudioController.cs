...
public sealed class AudioController
{
    ...
    private List<string> _playlist { get; set; }
    ...
    public async Task PlaySirenDisplay(List<string> paths)
    {
        if (_mediaplayer.IsPlaying)
        {
            Stop();
        }
        _playlist = new List<string>(paths);
        _mediaplayer = new MediaPlayer(_libvlc);
        _mediaplayer.EndReached += PlayNextSiren;
        _media= new Media(_libvlc,_playlist.First());
